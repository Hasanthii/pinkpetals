...
public interface OrderItemRepository extends JpaRepository<OrderItem, Long> {
    ...
    @Query("SELECT CASE WHEN COUNT(oi) > 0 THEN true ELSE false END FROM OrderItem oi " +
           ...
           "AND oi.order.status IN ('PROCESSING', 'SHIPPED', 'DELIVERED')")
    boolean hasUserPurchasedProduct(@Param("userId") Long userId, @Param("productId") Long productId);

    @Query("SELECT oi FROM OrderItem oi WHERE oi.order.user.id = :userId AND oi.product.id = :productId " +
           "AND oi.order.status IN ('PROCESSING', 'SHIPPED', 'DELIVERED')")
    List<OrderItem> findPurchasedItemsByUserAndProduct(@Param("userId") Long userId, @Param("productId") Long productId);
